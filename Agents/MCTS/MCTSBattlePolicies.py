...
class MCTSNode:
    ...
    def __init__(self, env: PkmBattleEnv, parent: MCTSNode = None, actions: list[int] = None, is_simulated=False):
...
class MonteCarloTreeSearch():
    ...
    def __init__(self, env: PkmBattleEnv, enable_print: bool):
    ...
    def expansion(self, node: MCTSNode, n_expansions=1) -> list[MCTSNode]:
        ...
        node.is_leaf = False
        if self.enable_print:
            print(f'Expanded {n_expansions} nodes: {node.actions} -> {node.children}')
    ...
    def backpropagation(self, nodes: list[MCTSNode]) -> None:
        ...
        for node in nodes:
            ...
            # Case of print
            if self.enable_print:
                path_utility_list = [str(str(n.utility_playouts)+'/'+str(n.total_playouts)) for n in backprop_nodes]
...
class MCTSPlayer(BattlePolicy):
    ...
    def __init__(self, name: str, enable_print=False):
        super().__init__()
        self.name = name
        self.enable_print = enable_print
    ...
    def get_action(self, state: GameState) -> int:
        ...
        tree = MonteCarloTreeSearch(state_copy,self.enable_print)
